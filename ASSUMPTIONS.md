...
## 2026-08-10 — Shutdown clears a record whose process is provably gone

- **Decision:** `dev:down` releases a PID record when the operating system reports `ESRCH` for that PID _and_ the configured port has no listener under it. No signal is sent on that path; the record file is simply removed. A PID that exists but is not verifiably owned still refuses with `PID_OWNERSHIP_REFUSED`, unchanged.
- **Reason:** observed as four simultaneous `PID_OWNERSHIP_REFUSED` failures in `test-integration` after the host lost the previously started services. Without this path, any crash, sleep, or killed terminal leaves records that make shutdown fail forever and require a human to delete files by hand, which contradicts the requirement that cleanup after a crash be idempotent. A new `processAbsent` helper distinguishes `ESRCH` from `EPERM`, because the existing `processExists` treats every `kill(0)` failure as absence — safe for "may I signal this?", unsafe for "may I forget this?".
- **Verify later:** the `crash-cleanup` integration case kills all four owned services with `SIGKILL`, asserts the records survive the kill, asserts `dev:down` clears them, and runs `dev:down` a second time to prove idempotence. If a supervisor or control socket ever replaces PID signalling, this release path must keep requiring proof of absence rather than trusting a record.

## 2026-08-10 — An orphaned owned process is reconciled by verified identity, never by a sweep

- **Decision:** when a service's PID record is missing but its port still has a listener, the holder is identified with `ps -o command=` and `lsof -d cwd` before anything is signalled. A process is stopped by exact PID only when its executable path, script path, port argument, and working directory all lie inside this repository. Anything else is left alone and reported.
- **Reason:** the earlier crashed run left `fixture-server.mjs --port 4183` alive as PID 60038 with no record, so `dev:down` skipped it silently and `dev:preflight` then reported `FOREIGN_PORT_LISTENER port=4183`. The system was right to refuse: a record is not authority, and this host runs fifteen sibling repositories. Verifying identity from the operating system and signalling one PID is the reconciliation `GOAL.md` §0A.2.5 prescribes; `pkill`, `killall`, and every broad sweep remain prohibited.
- **Verify later:** consider having `dev:down` report an unrecorded but repository-owned listener explicitly, instead of leaving it for the next `dev:preflight` to surface. The diagnostic exists today but arrives one command later than it could.
