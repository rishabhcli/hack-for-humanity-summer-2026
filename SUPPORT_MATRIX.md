...
The domain rules behind those refusals are implemented and tested even though no camera surface
exists yet: the quality gate, the measured/withheld outcome type, the repeatability and
change-claim layer, and the citation-gated threshold registry. `docs/INVARIANTS.md` records which
invariant each one encodes and the named property tests that attack it. **Implemented domain rules
are not a validated measurement**, and none of the rows above becomes "supported" until the
known-angle validation in `validation/` produces evidence.
...
| Canonical verification | Green: `verify-all passed steps=12` locally and in CI         | GitHub Actions run 31427300557 on commit `dade84b`; re-run by `npm run verify-all`   |
| Shipped copy           | Diagnosis, clearance, and treatment language refused          | `npm run check:copy`; three rules, containment-matched disclaimers, observed failing |
| Frame egress           | No network or frame-serialization API in shipped source       | `npm run check:egress`; 15 prohibited names, zero exemptions, observed failing       |
